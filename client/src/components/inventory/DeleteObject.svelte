<script lang="ts">
  ...
  const handleDelete = () => {
    deleteRequest<any>(`inventory/${object.code}`)
      .then(_ => dispatch("delete", object))
      .catch(err => {
        error = err.message;
        showError = true;
      });
...
<div class="col-span-12 mb-4">
  <div class="text-xl font-bold text-center">
    <span class="text-slate-900">Esta seguro de eliminar el objeto {object.type} {object.code} ?</span>
  </div>
</div>
