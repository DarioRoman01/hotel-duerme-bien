    ...
    <div class="relative inline-block rounded-md border-2 border-slate-900 w-full">
      ...
      {#if show}
        <div in:slide="{{duration: 500}}" out:slide="{{duration: 500}}">
          <div class="w-full max-h-32 overflow-y-scroll no-scrollbar">
            <div class=" w-full p-1">
              <button class="bg-gray-50 hover:bg-slate-900 hover:text-slate-50 text-slate-900 font-bold p-2 rounded-md w-full" on:click={handleDefaultClick}>
                {title}
              </button>
            </div>
            {#each options as option}
              <div class=" w-full p-1">
                <button class="bg-gray-50 hover:bg-slate-900 hover:text-slate-50 text-slate-900 font-bold p-2 rounded-md w-full" on:click={() =>handleOptionClick(option)}>
                  {option}
                </button>
              </div>  
            {/each}
